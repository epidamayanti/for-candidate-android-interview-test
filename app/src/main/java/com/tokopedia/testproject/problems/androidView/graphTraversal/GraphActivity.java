...
public class GraphActivity extends AppCompatActivity {
    ...
    private void setupAdapter(Graph graph) {
        ...
        traverseAndColorTheGraph(graph, graph.getNode(1), 2);
    }

    private void traverseAndColorTheGraph(Graph graph, final Node rootNode, final int target) {
        ...
        adapter = new BaseGraphAdapter<ViewHolder>(this, R.layout.node, graph) {
            ...
            public void onBindViewHolder(ViewHolder viewHolder, Object data, int position) {
                viewHolder.textView.setText(data.toString());
                Log.d("node", "onBindViewHolder: "+rootNode.getPosition());

                if(init == 0){
                    if(target == position){
                        viewHolder.cardView.setBackgroundColor(Color.parseColor("#8600C8"));
                    }
                }
